...
#ifndef _Included_org_synergy_injection_Injection
#define _Included_org_synergy_injection_Injection
#ifdef __cplusplus
extern "C" {
#endif
 ...
 */
JNIEXPORT void JNICALL Java_org_synergy_injection_Injection_start (JNIEnv *env, jclass thiz, jstring deviceName) {
  struct input_id id = {
  ...
  /*
  if(0<uinput_fd)
      __android_log_print (ANDROID_LOG_INFO, DEBUG_TAG, "open uinput_fd success  id:%d ",uinput_fd);
  else
      __android_log_print (ANDROID_LOG_ERROR, DEBUG_TAG, "open uinput_fd fail  id:%d ",uinput_fd);
  */
 ...
 */
JNIEXPORT void Java_org_synergy_injection_Injection_stop (JNIEnv *env, jclass thiz) {
...
JNIEXPORT void JNICALL  Java_org_synergy_injection_Injection_keydown (JNIEnv *env, jclass thiz, jint key, jint mask) {
...
JNIEXPORT void JNICALL  Java_org_synergy_injection_Injection_keyup (JNIEnv *env, jclass thiz, jint key, jint mask) {
    int translatedKey = keycode (key);
    if (translatedKey < 0) {
        // Unknown key...
        return;
...
JNIEXPORT void JNICALL  Java_org_synergy_injection_Injection_movemouse (JNIEnv *env, jclass thiz, const jint x, const jint y) {
    suinput_move_pointer (uinput_fd, x, y);
}

JNIEXPORT void JNICALL  Java_org_synergy_injection_Injection_mousedown (JNIEnv *env, jclass thiz, jint buttonId) {
    suinput_press (uinput_fd, BTN_LEFT);
}

JNIEXPORT void JNICALL  Java_org_synergy_injection_Injection_mouseup (JNIEnv *env, jclass thiz, jint buttonId) {
    suinput_release (uinput_fd, BTN_LEFT);
}

JNIEXPORT void JNICALL  Java_org_synergy_injection_Injection_mousewheel (JNIEnv *env, jclass thiz, jint x, jint y) {
