                         ...
                         uint16_t type, uint16_t code, int32_t value)
{
  ...
  if ( write(uinput_fd, &event, sizeof( event))!= sizeof(event) ){
      __android_log_print (ANDROID_LOG_INFO, DEBUG_TAG,
                           "Failed to write(uinput_fd, &event, sizeof(event)--%d--%d--%d\n",type,code,value);
...
int suinput_open(const char* device_name, const struct input_id* id)
{
  ...
  for (i = 0; i < UINPUT_FILEPATHS_COUNT; ++i) {
    uinput_fd = open(UINPUT_FILEPATHS[i], O_RDWR);
    //__android_log_print (ANDROID_LOG_INFO, DEBUG_TAG, "filepath :%s, uinput_fd  :%d ",UINPUT_FILEPATHS[i],uinput_fd);
    if (uinput_fd != -1)
      break;
  }
...
int suinput_move_pointer (const int uinput_fd, const int32_t x, const int32_t y) {
  if ( -1 == suinput_write(uinput_fd, EV_REL, REL_X, x) ){
      __android_log_print (ANDROID_LOG_INFO, DEBUG_TAG, "move operation failed ");
      return -1;
  }
...
int suinput_press(int uinput_fd, uint16_t code) {
  __android_log_print (ANDROID_LOG_INFO, DEBUG_TAG,"press operation success " );
...
int suinput_click(int uinput_fd, uint16_t code) {
  if (-1 == suinput_press(uinput_fd, code) ){
      __android_log_print (ANDROID_LOG_INFO, DEBUG_TAG, "click operation failed ");
      return -1;
  }
