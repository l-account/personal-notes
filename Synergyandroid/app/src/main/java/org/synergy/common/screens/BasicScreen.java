...
public class BasicScreen implements ScreenInterface {
    ...
    public BasicScreen (boolean isPrimaryClient) {
    	...
    	this.m_isPrimary=isPrimaryClient;
    }
	...
	public void keyUp (int id, int mask, int button) {
		if (button < buttonToKeyDownID.length) {
			...
			Log.debug("found keyUp button parameter  "+keyDownID);
		} else {
	...
	public void warpCursor(short x,short y){
    	assert (m_isPrimary);
    	//todo    get android point x y
    	mouseX=x;
    	mouseY=y;
	}
